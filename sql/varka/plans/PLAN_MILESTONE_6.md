...
| 189 | A dense loop enters a C2 deoptimization cycle at twelve outputs. **Planned** (`PLAN_TASK_189.md`, 26 September 2026): the admission check is a census of fresh JVMs read from the JVM's compile and trap log (`dev/varka_deopt_cycle.sh`), and it says the form every query runs since task 87 reached the cycle in none of eighty forks, while the single-epilogue form before it cycles in 39 of 40 at both widths - reproducible on demand, so the mechanism hunt has its fork, and the fix is not built unless the default-form sweep or task 212's path to C2 reaches the cycle. *Lead from `PLAN_TASK_87.md` 9.5: in the fork where the single-epilogue form's null-free rows sat in the cycle at 128 bits, the per-group form's did not* | 2.12, from task 87's benchmark | medium |
