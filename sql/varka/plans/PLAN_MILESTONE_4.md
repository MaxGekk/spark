...
is its exit - the row closes with (C), the README, which is written from (B)'s
files - and task 78's public ratio waits on the same runner. Five open rows had
nothing waiting on them and were moved to `PLAN_MILESTONE_5.md`, text and task
numbers unchanged, on the 4 September precedent: 25 (ILP, whose harness stopped
measuring what it was built for), 72 and 73 (emitter infrastructure, where 73's
...
Moved to `PLAN_MILESTONE_5.md` section 2.24 on 11 September 2026, when the
milestone was re-scoped a second time to what closes it - task 62's
measurement and what it needs; the text is there unchanged. The heading stays
so citations of this section number still resolve.
...
**Step B2 is built and is the default** (`PLAN_TASK_32.md` 7.6). `groupOutputs` admits an
output past `GROUP_BUDGET` when joining lets it skip a prefix the group already computes, up
to `FUSED_CEILING` = 400; the four fields over one date are one loop method of 52 ops
with one prefix, at 1762.6 M rows/s against 818.0 with a method each (2.15x
at AVX-512, 2.51x at 128-bit), two fields 1.60x and three 1.91x. The weights
were recounted first, from emitted instructions, because they now bound a method: the
prefix is 31, the field tails 4 to 7, and `add_months`' tail 81 - the finding that set the
ceiling, since it and not the fields decides how many outputs one method admits. Task 17's
plain-chain case stays split, asserted by construction over a corpus of non-calendar
shapes, and the greedy order's one limitation (`year(d), year(d2), month(d)`) is pinned as
a limitation. Task 58's debt closes on the way: `weekofyear` and `yearofweek` over one
shift share a method now, 813.8 to 1401.8 M rows/s.
...
**Task 44's half moved to `PLAN_MILESTONE_5.md` 2.18 on 11 September 2026**,
where task 87 absorbed it: that task has the epilogue exceeding the JVM's
65535-byte method cap, diagnoses the cause as the epilogue not being
partitioned the way `GROUP_BUDGET` partitions the loop, and therefore has to
choose the mechanism row 44 would have chosen. Row 44's size ladder and its
`HugeMethodLimit` measurement are requirements there. Task 43's half is done
and its text below is unchanged.
...
throw happens before the kernel. *Corrected by `PLAN_TASK_59.md` 2: the row
engine never parses the name when the date is null, so a pre-pass that threw
would err where the row engine returns NULL; under ANSI the leaf declines the
batch and the row engine raises.* The IR sees `NextDay(days, ColumnRef(k))`:
...
**What it removes, and the correction the task's own plan made to it.** For
a qualifying root, the loop's per-group `orValidityBitsAt`, per root. The
per-group `validityBitsAt` read and the word locals are a separate and
stricter question, per loop *method*: a word survives if and only if some
node in that method still consumes it. This section first said "where every
root of a loop method qualifies", and `PLAN_TASK_70.md` 2.2 shows that is
neither necessary - a method of `year(d), greatest(d, d2)` needs no word once
the pass serves both roots - nor sufficient, which is the half that would
have been a bug: a method of `year(date_add(d, off))` alone has one
qualifying root and still needs the word, because task 52's range guard ANDs
its condemning mask with it. Built to the rule as first written, that shape
would decline batches on nullable data with a stray value in a null slot -
right answers through the row engine, and the kernel's win gone on exactly
the shape the guard was built for. Where no word survives, the masked body
does become byte-identical to the dense one, so the shape needs one body
rather than two - a code-size and compile-time result on top of the
throughput one, and the part of this task to verify rather than assume.
...
known-true mask, which is computed per group; a node that can be null
on valid inputs - task 42's `make_date`, task 63's `try_*` forms - breaks
the invariant the derivation rests on; and a range guard (tasks 42, 52, 60)
consumes the word of a node that may itself qualify, which is the third
consumer above and the reason the two questions are separate. Those keep
their per-group call, and task 46 is what makes it cheap. The two compose;
this one is measured on a tree that already has 46, and what is left over is
what task 47 is finally worth.

**What it is worth, as a bound - requoted on 7 September 2026 from the parity
file task 32 B2 regenerated, since the figures this section first carried
predate task 46.** The masked rows cannot pass their dense counterparts, and
after this task they should approach them. Task 46 took most of the
single-field gap already; what is left concentrates in the multi-output
methods B2 now emits by default and at 128-bit:
...
60's guard consumes it - see `PLAN_TASK_70.md` 2 for that inventory and for
task 64, which removes the guard from the in-range case and so widens what
this task can drop.

**Update, when task 70 landed** (`PLAN_TASK_70.md` 9). Step one paid: the
four-field shape B2 emits by default runs 1.59x and 1.89x faster masked than
the per-group arm that shipped, and sits on its dense twin; and the one-body
claim this section asked to have verified holds exactly - for every shape
whose words all die, the masked loop and epilogue are the dense ones' bytes,
on the whole output ladder, so the epilogue's `HugeMethodLimit` crossing is
the dense one's now (49 shared, 22 unshared). Two things this section had
wrong are corrected above and in the task's 2.2 and 3.3: the liveness rule is
per method, not "every root qualifies", and the picks' null substitution is a
fourth consumer of the word that keeps both their reads, which is why
`greatest(d, d2)` closes 47% and 19% of its gap where the calendar shapes
close all of theirs. What remains per lane group is the task's 9.5 and is task
47's; what remains per batch on short batches is the masked driver's
null-state prologue, which is new to 47's list.
...
### 2.35 `GROUP_BUDGET` retuned on the emitter as it is (task 71)

Added 7 September 2026, out of task 32 step B2's regeneration
(`PLAN_TASK_32.md` 7.6 and the debt register). The budget of 16 rests on one
measurement: task 17's pair, two outputs over a shared eight-op chain, ran
1.4x faster as two loop methods than as one, and that was read as register
pressure - recomputing eight ops in registers beats the wider method. The
parity file carried both rows ever since so that a retune would be measured
...
inline, and register pressure was never the binding constraint at 24 ops.

**What the task decides.** Whether 16 is still the budget, on a ladder rather
than on one pair: 16, 24, 32, 48 and 64 over the shapes where a wider method
changes what is emitted - task 17's pair, two chains over a shared subchain,
the `CASE WHEN` arms and the DAG-CSE section's outputs, the mod-7 pair - at
both widths, with `-XX:+PrintCompilation` beside the throughput per the
standing rule. Two constraints B2 measured come in as inputs rather than as
open questions: C1 refuses a loop method past about 1900 bytes ("out of
virtual registers in LIR"), so a method past that runs interpreted until C2
lands - 340 ms at 376 ops, once per shape per JVM behind the class cache -
which is a startup cost the ladder has to price, not only a steady-state
one; and B2's clause 2 was deliberately restricted to prefix reuse
(`saved > 0`) rather than to "the marginal cost fits", the wording task 32's
3.2 first wrote and 10.2 rejected on task 17's evidence, so if the budget
moves, whether clause 2 should widen with it is the same measurement's second
column, not a separate task. Whatever the ladder says, the byte-identity
guard B2 added for non-calendar shapes stays as the assertion that a budget
change changes exactly the shapes it names.

**What it does not do.** It does not touch the fragment mechanism, the
ceiling, or a single output's internal width (task 43 measured that and left
the decision to the emitter); it does not revisit `MAX_FUSED_NODES`.

### 2.36 Output order for prefix affinity (task 72)

Moved to `PLAN_MILESTONE_5.md` section 2.25 on 11 September 2026, when the
milestone was re-scoped a second time to what closes it - task 62's
measurement and what it needs; the text is there unchanged. The heading stays
so citations of this section number still resolve.
...
Moved to `PLAN_MILESTONE_5.md` section 2.26 on 11 September 2026, when the
milestone was re-scoped a second time to what closes it - task 62's
measurement and what it needs; the text is there unchanged. The heading stays
so citations of this section number still resolve.
...
**Corrected on 10 September 2026 by `PLAN_TASK_76.md` 10.** Two things below are
wrong. The design proposed here - a rule on the number of validity writes - is
right at 4 lanes and wrong at 8 and 16, where the width-named helpers win at
every write count; the loss is a 4-lane regime, that being the one width where a
validity group is half a byte so two consecutive groups rewrite the same byte.
And the reason the helpers exist, a refused inline of the 212-byte general
writer, does not apply to this shape: the compiled loop inlines the writer in
both arms, and the arm that loses is the smaller one doing the cheaper address
arithmetic. The task closed with a recorded decline and the finding went to row
47. The rest of this section is left as it was written.
...
crosses the read-back floor anyway. The 1B-row run puts it at 0.66x of stock
with a columnar consumer and 0.57x counted (`PLAN_TASK_62.md` 10); the
43.2-against-67.7 pair this section first quoted is the laptop's, from 9.1, and
is superseded. The predicate's own kernel runs at 681.2 M rows/s when nothing
is selected.
...
**The cause is three layers deep** - this section said two when it was written,
and `PLAN_TASK_78.md` 2.2 found the third while checking the first two. The
first two are as below; the third is that the row conversion is
`UnsafeProjection.create(childOutput, childOutput)`, so every selected row is
serialised over *every* child column and the projection above discards the one
it did not want. That layer is inside the floor rather than beside it, and it
is why pruning the node's output removes work rather than merely moving an
operator. `PLAN_TASK_78.md` 2.1 also corrects the shape's description: the
losing family is a predicate over more columns than its consumer wants, which
is why exactly the two-column predicates lose and every one-column predicate
is fused - Spark's own column pruning makes the narrowing projection redundant
in the one-column case before any columnar rule runs. The
...
branches whatever the condition says. So a guarded node inside a `CASE` arm
declines batches on rows the condition sends to the *other* arm. Both
`CaseWhen` and `If` over a column-count `AddMonths` fuse today, so it is
reachable rather than theoretical.

*Corrected on 8 September 2026 by `PLAN_TASK_79.md` 2.2.* This paragraph used
to motivate the task with `CASE WHEN m BETWEEN -1000 AND 1000 THEN
add_months(d, m) ELSE NULL END`, and to say the cliff "defeats exactly the
range test the user wrote to keep the shape fused". That projection does not
fuse at all - `nothing fused: every entry declined` - because a bare
`IntegerType` column has no arm in predicate position, so the user cannot write
that range test in a fusable form. The cliff is real and reachable through a
date- or field-driven condition (2.3 there has both shapes, with their IR), but
it is not the user's own bound being ignored, and the weaker claim is what the
design has to be worth. Answers stay correct, because the row
...
Moved to `PLAN_MILESTONE_5.md` section 2.27 on 11 September 2026, when the
milestone was re-scoped a second time to what closes it - task 62's
measurement and what it needs; the text is there unchanged. The heading stays
so citations of this section number still resolve.
...
| 43 | What bounds a loop method inside one output. **Measured; the decision waits on the emitter** (`PLAN_TASK_43.md` 8) - a 20-to-248-op ladder in one loop method shows no cliff at either width: nanoseconds per row per op flat to +-4% at AVX-512 and improving then flat at 128-bit, and tier-4 compile linear at ~1.1 ms/op, 271 ms standard and **186 ms OSR** at 248 ops against the ~10 seconds at 64 ops the `GROUP_BUDGET` javadoc still cites. Two of four predictions missed: 128-bit does not degrade, and C1's virtual-register refusal is width-independent and lands on the epilogue, never on a loop. **DONE 10 September 2026**: the second half was the javadoc correction 8.5 deliberately deferred, and it waited on the decision half, which task 71 settled. `GROUP_BUDGET`'s javadoc no longer cites the ~10 second compile as its measured reason - the observation is kept and the inference dropped, since the honest reading is a compile *queueing* under load rather than ten seconds of compiler work, and a queued compile can bite at any width and any op count, which no per-method budget can bound. What justifies 16 now is task 71's survey instead, and the javadoc carries 8.5's limits rather than swapping one confident number for another | The cliff located first - single-output loops at 60 to 250 ops, throughput and time-to-peak - then split, decline or accept, chosen on that number | A committed number per width; whichever mechanism wins, `CASE WHEN year ELSE month` either fuses within a stated bound or declines with a recorded reason |
...
| 47 | One validity write per word. **DONE - shipped as an option, default off** (`PLAN_TASK_47.md` 10). The admission check passed: Arrow's validity buffer carries whole 64-bit words at every length, the driver zeroes exactly the outputs this writes, and `groupOutputs` gives each output one writer - so a plain store may replace the OR. The word writer is correct and it is a **6 to 9% win at 4 lanes** at one and two writes, which is where task 76 said the read-modify-write serialises on a shared byte, and where its inversion of task 46's helper choice disappears under it. It is an **11 to 20% loss at 8 and 16 lanes**, where a group owns whole bytes and there was never a chain to remove: an eight-byte store plus an accumulator, a mask, a shift and a branch is more work than a one-byte read-modify-write whose helper inlines. Production runs at 16 lanes, so the default stays off and this row's title is not, as written, an improvement. The rule the numbers do support is keyed on the bit layout rather than the write count - a validity group smaller than a byte, which is 2 and 4 lanes and nothing else - and is `PLAN_MILESTONE_5.md` 2.23, task 92, together with option B and section 3.4's driver item | Bits accumulated in a register and the whole 64-bit word stored, so the *read* goes and the store count stays - which is what makes the A/B price one change. The destination segment is sized to whole words for the outputs this writes; the epilogue keeps its per-batch read-modify-write, so no accumulator crosses the loop/epilogue method boundary. Storing once per word instead of once per group is option B of `PLAN_TASK_47.md` 3.1 and is not what shipped | The word's existence, checked against Arrow rather than assumed, and the single-writer claims read off the emitter; the two arms compared bit for bit over a length ladder through both sides of every word boundary at three widths and three null states, plus a differential over a 5000-row fixture whose second batch is fourteen words and eight rows; and the ladder's own rungs shown to be one shape family before their numbers are read |
...
| 70 | Validity as bitmap algebra in the driver (section 2.34). **DONE** (`PLAN_TASK_70.md` 9: the four-field shape 1.59x at AVX-512 and 1.89x at 128-bit against the arm that shipped, and on its dense twin; every served row faster at both widths; on by default; a masked method whose every word is dead is its dense twin's bytes, and the epilogue's crossing moves to 49 shared and 22 unshared) | The analysis that says whether a root's validity word is a pure AND/OR over input bitmaps (no `Cond`, no `IfElse`, no node that can be null on valid inputs); the driver's per-batch bitmap pass, emitted between the null-state prologue and the all-null shortcut - a copy for one column, a bytewise AND or OR for several, `setValid` or `zero` where an operand's null count decides the whole expression, the final partial byte masked to `length % 8` bits - behind its own switch; the per-group write not emitted per served root, and the per-group read and the word locals not emitted per loop method in which no consumer is left - a guard, a blend, a `Cond` root's OR or an unserved root, "every root qualifies" being neither necessary nor sufficient (`PLAN_TASK_70.md` 2.2); the one-body-not-two consequence for such shapes verified rather than assumed | The differential at both widths and both switch settings with byte-identical validity, the existing suite as the oracle; `assertSameOutput` holding on the tail byte; the qualifying predicate tested against a `CASE WHEN`, a filter root and a null-on-valid-input node, each of which must not qualify; `year(date_add(d, off))` over poisoned null slots keeping its word and declining nothing, which is the guard case the first draft of section 2.34 would have broken; a null-free operand under an OR root and an all-null one under both operators, the three states resolved rather than assumed; the driver's own code size pinned on the output ladder beside the epilogue's, since it is the method every batch runs; the masked parity rows measured on a tree that already carries task 46, with a registered bound that no masked row passes its dense counterpart |
| 62 | The closing measurement: every date expression, on a 512-bit datapath, against stock Spark on JDK 17 and JDK 25. **(A) done** (`PLAN_TASK_62.md` 9 and 10: the driver, its module and shell driver, and the laptop's four files - re-measured at 1B rows, where the 5% job-size rule is met with the worst row at 4.5% and the median at 1.7%, every one of 50 shapes fused, and three of the four filter losses turned into wins by task 37); (B) **done** (`PLAN_TASK_62.md` 11.17): measured on an AMD EPYC 9V45 whose datapath probe reads 2.01 in the same file that carries the numbers, at 2e8 rows fully resident with a worst fixed share of 3.5%, where Varka runs the twelve chains of `Chains.ENTRIES` at 6.9 to 11.2 ns/row for **9.4x to 14.2x over stock Spark 4.2**, median 10.2x, with the engine-off column agreeing with stock throughout. Three findings came out of getting there and are the reason the section is long: the gate certified a machine that did none of the work, because every GitHub job is a fresh VM and the probe sat in the wrong one (11.15); the surface cannot answer the datapath question at all, since a third of it is memory-bound, which is why the measurement is `Chains` and not `Surface` (11.13, 11.16); and the 256-to-512 step is worth about 1.14x once the three scalar arms are used to subtract the machine, not the 2x the lane count implies, so these kernels are latency-bound rather than issue-bound and task 92 now has a number to confirm. The speedup over stock *falls* with expression depth rather than rising as 11.13 argued - the surface's larger ratios are mostly Spark's per-row overhead, and about 10x is what the arithmetic is worth once that is amortised away - and **(C) done** (`PLAN_TASK_62.md` 11.18): the README's benchmark section rewritten from the committed surface and chain files with a reproduction guide taken from the CI job's own commands, leading with about 10x on compute-bound expressions rather than the surface's larger single-call ratios, keeping the three losses in the table, and quoting `trunc(d, 'QUARTER')` against the engine-off column because it is the one row of fifty where the fork's row engine and 4.2.0's differ by more than 20% | A Java driver under `sql/varka/bench` submitted to three distributions - stock Spark on JDK 17, stock Spark on JDK 25, this fork on JDK 25 - in one dispatch of the benchmark workflow with `expected-cpu` pinned to a full-width Xeon, running one committed SQL query per covered date expression in the projection and filter shapes over a table sized so the per-job fixed cost is under 5% of every Varka row's wall time (at least 200 ms per Varka query), recording executor time beside wall time for every row, with provenance including the 256-to-512 op-count ladder that proves the datapath; `dev/varka_bench_surface.sh` and the diff script producing the table; README's benchmark section rewritten from the three files with a reproduction guide a reader can follow from the downloads alone; the laptop's run committed as the second data point | Three results files with provenance, generated by one workflow dispatch on a 512-bit runner; every README figure tracing to them (the quote check); every Varka row's fixed share (wall minus executor time, over wall) under 5% in the files; the fork-with-Varka-off row agreeing with stock Spark on JDK 25 within noise on every shape; the ladder in the provenance showing the 256-to-512 step near 2x, or the file labelled 256-bit |
| 71 | `GROUP_BUDGET` retuned on the emitter as it is (section 2.35). **DONE** (`PLAN_TASK_71.md` 10), and the number did not move: it is still 16. The budget bounds the *method*, while the marginal cost it is compared against already excludes nodes the group holds - task 17's pair is 14 + 6 against 16, split into two methods costing 28 nodes of work where one costs 20. The merge is strictly less work and clause 1 rejects it anyway. What was missing is the exception step B2 already wrote, generalised: `shareWholeNodes` lets an output reusing whole nodes join too, which merges exactly the shapes a budget of 24 would and nothing else, at the shipped budget. Raising the budget instead would have loosened the bound that keeps compile time in hand | A static survey first: of nine shapes only three regroup across 16 to 64, and only one above 24 - for a saving of one lane op out of 38, at the cost of the one pinned assertion that spells its arithmetic out. So the boundary is 16 to 24 and the variable is the rule. `VarkaEmitOptions.shareWholeNodes` on by default; the parity A/B rewritten from budget-versus-budget to rule-off-versus-on, since with the rule shipped the old split arm merges too and the pair would have become two copies of one kernel; both files regenerated. Three javadoc readings retired with it: that the reversal was `orValidityBitsAt` inlining, a call task 70 stopped emitting for this shape; that the retune was task 43's; and section 2.35's belief that B2's byte-identity test already guards a budget change, which it does not - it compares prefix-sharing off against on at one budget | The mechanism established before any number was chosen, and it refuted the plan's own first prediction; the re-pinning cost enumerated by making the change and running the suites rather than by grepping, which found one assertion rather than the predicted ten; the guard 2.35 believed existed, written; the rule asserted method for method in both directions over a ten-shape corpus, with the sharing half required to differ from the shipped setting so it cannot stop exercising the rule; 269 catalyst and 180 sql/core tests green, the differential unmoved; and a regeneration read by task 77's band and gate - no row past the band, seventeen unreadable rows set aside, controls within 2.4%, gate clean, requote empty |
| 76 | The validity helper choice, keyed on the loop body (section 2.38). **DONE - declined** (`PLAN_TASK_76.md` 10), which 2.38 licensed and the measurement chose. The loss is a 4-lane regime and nothing else: at 8 and 16 lanes the width-named helpers win at every write count by 8 to 36%, and 4 lanes is the one width where a validity group is half a byte, so two consecutive groups read-modify-write the same byte. A rule on the write count is right there and wrong at the other two widths. What the shipped default costs is 4 to 7% on a 4-lane body with one or two per-group writes - inside a loop shape task 47 replaces | Three explanations excluded from the JVM's own output, including the one task 46's javadoc rests on: the compiled masked loop inlines the writer in *both* arms, so there is no refused call; the arm that loses is the smaller one, 642 instructions against 654; and it is the arm doing the cheaper address arithmetic, shift-and-mask against signed divide and modulo. What differs is register-file placement - ten extra general-to-vector moves in one arm against ten more general-register ALU ops in the other. Also fixed on the way: `VarkaEmitDump`'s `--options` parser silently read any non-`true` string as `false`, so `validityByWidth=on` selected the arm it was meant to exclude | The arms asserted to emit different kernels before any number was read, which is the failure this task was downstream of; a write-count ladder holding the shape family constant rather than a threshold fitted to the two shapes that raised it; ten runs across three widths; and the finding carried to row 47 rather than spent on a rule inside a shape that is scheduled for replacement |
| 77 | The 128-bit compile cliff behind the per-group validity OR (section 2.39). **DONE** (`PLAN_TASK_77.md` 10), and nothing it shipped is what the row was named after. 2.39's reading was counting the wrong element - the traps it reported are guards C2 *inserts* per compile, not deoptimisations that happened, and its own arithmetic gave it away, nine per method being exactly the eight compiles plus the tier-3 compile it counted two paragraphs earlier. Corrected from a run made for the purpose: a storm is real, at 957 runtime deoptimisations with one method taking 100 across 194 compiles, and that method is *hand-written*; the row's own shape has recovered; and benches 930 and 945, one expression with the validity OR and without, storm identically, which retires the lowering as a suspect. The cause is milestone 5's task 90 | 2.39 corrected along with three records that depended on it (the debt entry's `PLAN_TASK_11.md` citation, withdrawn twice over; row 46's +75%, which is one width of a two-sided move whose other half was the collapse; the unserved-root list, missing the `try_*` forms and the subtree closure). `dev/varka_trap_census.py`, which separates the two elements that share the name `uncommon_trap` so the mistake cannot be repeated. The row key in `dev/varka_bench_diff.py`, which was `(table, case)` and not unique, so 207 rows parsed to 206 and `--requote` silently skipped one. And `dev/varka_bench_gate.py`: the invariants a results file must satisfy whatever its numbers are, each pair derived by holding in every committed revision rather than asserted. The band half of this work belongs to task 90 and is credited there | The census reproduced from the JVM's own output, with a probe showing a loop that never deoptimises emitting four of the traps 2.39 counted; the keying fixed with a self-test over the collision the parity file actually holds; and the gate walked back over 76 committed revisions of four files, flagging three and only three - `aef0b82260e` and the two regenerations committed while it was still broken, all at 128-bit - which is what turns 'a committed row that would have caught the collapse the day it landed' into a checked claim |
| 78 | A forwarded-only projection over a Varka filter runs through rows (section 2.40). **DONE** (`PLAN_TASK_78.md` 9.6: measured on the laptop at 1e9 rows, `WHERE d < d2` with a columnar consumer goes 0.59x to **1.14x** - the loss becomes a win - while the same predicate counted improves 0.56x to 0.80x and stops at the read-back floor, and `d IS NOT NULL` counted is unchanged at 0.45x because it is a one-column predicate that never had a narrowing projection to absorb. The README said all three were one bug; it now names the two causes.) (`PLAN_TASK_78.md`): the rule absorbs the narrowing projection into `VarkaFilterColumnarToRowExec`, which removes the operator boundary and stops the node converting columns the consumer discards; the surface's three two-column predicates are ordinary `filter` entries again. On the laptop (`VarkaNarrowingBenchmark`, both widths) the losing shape runs 2.2x of the Janino session at 10% selected, 1.5x at 70% and 1.3x at 100%, and the two predictions that missed both said the read-back floor would still hold it under stock - it does not, which is a finding about task 19's floor rather than about this shape and is why scope item 13 keeps its own justification. The ratio for the public table waits on a **laptop regeneration of the surface**, not on a runner: `PLAN_TASK_78.md` 9.5 shows the surface cannot meet the job-size rule on CI at any row count, because a runner's 36 ms constant would need 0.85e9 to 1.37e9 rows for its lightest third and that is 21 to 33 GiB against a runner's 15 GiB. About four hours of a quiet machine, dominated by the three row-engine distributions; `--expect-fused` guards the three entries, which task 78 already returned to ordinary filter rows | `SELECT d FROM t WHERE d < d2` runs at 0.66x of stock Spark with a columnar consumer and 0.57x counted in the 1B-row run (`PLAN_TASK_62.md` 10; the 43.2-against-67.7 laptop pair this row used to quote is 9.1's and is superseded), because the rule takes a projection only when an entry fuses and a forwarded-only projection has none (task 62's run; see the debt register) | The two layers separated first, by one run of that row with the projection removed by hand: the rule leaves a Janino `Project` because a forwarded-only projection fuses no entry, and under that the node is not `CodegenSupport`, so it produces rows at task 19's floor against stock's fused loop. Then one of three, cheapest first: decline the shape in the rule, which recovers to 1.00x with no new machinery; absorb the narrowing into `VarkaFilterExec` or the rule; or fuse the boundary by making the node `CodegenSupport`, which is scope item 13's lever and the only one that can win rather than draw | The throughput row back above stock at 10%, 70% and 100% selected; the differential over that query through both a row and a columnar consumer; no other plan shape changing operator |
| 93 | **DONE** (`PLAN_TASK_93.md` 9). Guarded day shifts did not compose: two guarded producers under one calendar consumer decline, although each fuses alone (task 62 (B), choosing the mixed-type chain entries). **Planned** (`PLAN_TASK_93.md`). `dayofyear(add_months(last_day(date_add(d, i)), 1) + ymy)` reports `day range [-6156428, 12144157] leaves the calendar lowering's range`, and only the lower bound fails: 12144157 sits inside task 69's widened ceiling of 14766813 while -6156428 is outside the floor of -5394572. The floor is structural, not a precision limit like 69's ceiling - the narrowed decomposition runs on `days + NARROW_BIAS` and needs it non-negative - so widening it trades the ceiling headroom 69 just bought rather than adding any. The fault is where the guard sits: task 52 moved it to the producer, and with two producers the inner one spends the whole narrow range as its promise and the outer shift has nothing left | A `GuardedDay` IR node the compiler inserts wherever the running interval would leave the range, resetting it there - twice in one expression where the range runs out twice. In the IR rather than beside it, because the emitter never sees literal values and `VarkaShapeKey` omits them, so a placement carried alongside would let one shape be served another's guards. Re-armed only where *this node's own* shift is runtime-valued: a literal shift leaves the range for every row, so a check above it would decline every batch | The shape fuses and its differential matches the row engine; `year(date_add(d, 20000000))` still declines with its existing reason, asserted first; `dayofyear(last_day(date_add(d, i) + ymy) + ymm)` fuses too, which needs two re-arms and is what makes the rule's plurality testable; every shape that fuses today emits identical bytes; the entry in `Chains` measured with `expectFused = false` before the change and `true` after, so before and after are one row in one file |
| 79 | The guard and the untaken arm (section 2.41): a guarded producer under a `CASE`/`IF` arm condemns the batch from the arm the row never takes (task 60's review; see the debt register). **DONE** (`PLAN_TASK_79.md` 9: the arm context costs under 3% at both widths and is negative on both mixed-null rows, so it is under the benchmark's noise floor, and the differential's declined count goes from positive to zero where the extreme rows sit in the untaken arm; `make_date` turned out to be a fourth node kind reaching this cliff, and 2.41's disjunction rule was replaced by the conservative same-arm-chain one, which is buildable where the guard is emitted. The pair's absolute rates are not committed - two regenerations of the parity file disagreed with each other on rows this task does not touch, which 9.1 records and the debt register carries). Its 2.2 corrects this row's original claim that a user's own `BETWEEN` on the count cannot keep the shape fused - that projection does not fuse at all, since a bare int column has no arm in predicate position, so the reachable shapes are date- or field-driven conditions and the claim the fix has to be worth is the weaker one. **Widened by task 63** (`PLAN_TASK_63.md` 9.7): the ANSI overflow check disposes of its mask through the same `emitGuardCollect`, so a checked `IntArith`/`IntNeg` under a `CASE` arm is a third node kind with this cliff - `CASE WHEN d < DATE'2020-01-01' THEN i + 1 ELSE year(d) END` condemns a batch from a lane the condition sends to the ELSE arm. Answers stay right; the fusion is lost on exactly the data the check is there for | Either the arm's condition mask ANDed into the guard, which keeps the shape fused, or the node excluded from the guarded set so the compiler declines it with a reason at compile time; an A/B, since either changes emitted bytes on a shape the parity file measures. The honest fix's cost is not ordering - the condition is emitted before the arms, so its mask is already in a slot - but sharing: a guarded node used both inside an arm and outside it is emitted once, so its mask must be the disjunction over its use contexts or an unconditional consumer silently stops declining (2.41) | The differential over the two shapes `PLAN_TASK_79.md` 2.3 pins - a date-driven condition over a column-count `add_months`, and the same over task 63's checked `i + 1` - fusing and answering correctly over a fixture whose extremes sit in the untaken arm, with the declined count going from positive to zero; the shared-node case of 2.1, where the guarded node is used both inside an arm and bare, still declining, which is the silent-wrong-answer test; the emitted bytes of every shape without an `IfElse` unchanged; `Greatest`/`Least` explicitly out, with the reason in the code |
...
* **A shift above a guarded day producer declined conservatively, including upward,
  ...
  It was a debt because `weekofyear(date_add(d, off))` is an ordinary query shape that
  had gone back to running on rows, and because the conservatism was invisible: the entry
  declined with a day-range reason that read as if the day really could leave the domain.
  Task 69 gave the upward direction its own limit, and found the review's own observation
  too modest. `NARROW_MAX_DAYS` is `(1 << NARROW_ERA_K) - 1 - NARROW_BIAS`, the ceiling of
  the era step's `w < 2^NARROW_ERA_K` shift domain; the review proposed the multiply's
  overflow, `w * NARROW_ERA_M < 2^31`, as the looser real bound. Neither is it. `eraOf`
  adds one era when the magic undershoots, and that correction keeps the split exact past
  the point the multiply wraps; what ends it is an undershoot of *two* eras, at
  `w = 20161385`. So `NARROW_DECOMPOSE_MAX_DAYS` is epoch day 14766813 (29 February 42400)
  rather than the multiply bound's 13443003, and the headroom over the shipped ceiling is
  about 9,266 years instead of 5,600. Established the way section 2 demanded: the era
  identity asserted exact at the bound and failing one day past it, plus an exhaustive
  sweep of both lowerings against `java.time` over the whole extended range.

  Three of the four shapes flipped. The fourth, `weekofyear`/`yearofweek`, did not, and
  the reason is the one already written above: `ThursdayOf` shifts `+-3`, only its `+3`
  side was this task's to recover, and a shape declines on the union of its directions.
  The ordinary query shape that motivated the entry is therefore still residual, and it
  cannot be closed the same way: below zero `w` is read as about 4.29e9 and no correction
  rescues it, so there is no headroom under `NARROW_MIN_DAYS` the way there was over
  `NARROW_MAX_DAYS`. What closes it is the other lever - task 52's runtime guard comparing
  against a bound the compiler picks from the shift above the producer, one compare against
  a different immediate - which is `PLAN_MILESTONE_5.md` 2.22, task 91.
...
* **The guard walk does not stop at a node that re-bases the day, so a producer whose
  ...
  the row engine computes it. **Its practical reach was recorded here as small and that
  was wrong** (corrected 11 September 2026 by task 73's admission check): the claim was
  that `dayRange` returns `Unknown` for a mod-7 child and declines the entry at compile
  time anyway, so only direct IR reaches the over-guard. That holds for the
  `date_from_unix_date` route and not for `make_date`, which `dayRange` bounds from task
  42's published years *without looking at its children* - so
  `year(make_date(2020, 1, dayofweek(date_add(d, off))))` is ordinary SQL that fuses and
  carries the guard, 1274 to 1317 bytes in the masked loop. What makes it worth recording is that it is the same family as task 69's
...
* **The parity harness reads `next_day(d, k)`'s dense row below identical masked bytecode at
  128-bit, and an interleaved probe does not (task 70).** With task 70's pass on, the
  masked and dense loop methods of `next_day(d, k)` are the same 257 bytes, yet the
  committed file reads the masked mixed-null row 14.6% above the dense null-free row at
  128-bit (4003.7 against 3494.3), and the first regeneration read the same pair 12.3%
  apart; at AVX-512 the pair is the expected way round. Timed interleaved in one JVM over
  one L1-resident buffer the two methods run within 1% of each other, and
  `-XX:+PrintCompilation` shows both reaching tier 4 the same way. It is a debt because a
  reader of the file will take a masked row above its dense twin as a claim about the
  kernel, and it is not one. The likely mechanism is the harness: the dense arm streams
  `nfData` beside `kData` and the masked arm `mxData` beside `kData`, two 80 MB buffers whose
...
* **Task 46's width-named validity helpers are the right default for one shape
  ...
  **Swept 10 September 2026 with a recorded decline** (`PLAN_TASK_76.md` 10):
  ...
  lanes only - at 8 and 16 the width-named helpers win at every write count, by
  8 to 36%. Four lanes is the one width where a validity group is half a byte, so
  two consecutive groups read-modify-write the same byte and serialise on it. The
  entry's own hypothesis - a rule on the write count - is right at 4 lanes and
  wrong at the other two, where the general pair never wins anything. And the
  explanation the helpers were added on does not hold for this shape at all: the
  compiled loop inlines the writer in *both* arms, and the arm that loses is the
  smaller one doing the cheaper address arithmetic. What separates them is
  register-file placement, in a regime **task 47 removes rather than tunes** -
  one validity write per 64-bit word turns sixteen partial stores over eight
  bytes into one store. Row 47's gating on task 46 was the right way round, and
  this measurement is what it was waiting for.
...
* **A calendar field is computed once per output, not once per date.** **CLOSED by task 32
  step B2 (see 2.9 and `PLAN_TASK_32.md` 7.6)**: the loop methods now compute the
  decomposition once per date too, and the four-field shape runs 2.15x its
  one-method-per-field form. The history that follows is kept as written. **Half closed by
...
* **The parity harness carries a depressed-row cluster per regeneration.** Every
  regeneration of `VarkaEmitterParityBenchmark` since task 52 has shown one cluster of
  unrelated rows 10-30% below the committed file (once a single row at -99%), a different
  cluster each run - `sequential kernels`, the depth-4 chain rows, a hand-written `datediff`,
  a shared-decomposition row, `dayofweek, validity OR-ed per group` - while the rows a task
  is measuring reproduce within 2% between runs (task 61's two runs are the latest pair;
  task 60's before them). It is a debt because a reader of the committed file cannot tell a
  task's movement from the run's cluster without the second run, which every task since 52
  has had to make. **The reading half is closed** (10 September 2026, task 77): the band is
  measured over ten runs per width and committed beside each results file, and
  `dev/varka_bench_diff.py --band` classifies a move against the tier its own case sits in,
  so the second run is no longer what tells a task's movement from the run's cluster. Held
  out - band fitted on five runs, tested on pairs of the other five - that cuts false alarms
  on an unchanged file from 32.9% of rows to 5.6% at AVX-512 and from 11.4% to 2.1% at
  128-bit. The cluster itself is unexplained and stays open as milestone 5's task 90; what
...
* **`VarkaFilterExec` compacts string columns through the generic on-heap pass (task
  59).** A derived int32 leaf reads its source through a `VarCharVector`, and a fused
  ...
  (`numFallbackBatchesNonArrow`), answers correct, on the row path. A debt because every
...
* **`GROUP_BUDGET`'s retuning evidence reversed, and nobody noticed for a regeneration.** The
  ...
  method, and the budget's own retune is open again on evidence. Two more facts for whoever
  takes it: C1 refuses a loop method past about 1900 bytes ("out of virtual registers in
  LIR"), so a wide method runs interpreted until C2 lands (~340 ms at 376 ops), and B2's
  clause 2 was deliberately restricted to prefix reuse (`saved > 0`) rather than
  "the marginal cost fits", so widening it is part of the same question. **Adopted as task 71
  (section 2.35)**; B2 did not act on it (`PLAN_TASK_32.md` 10.9).
* **Greedy grouping leaves `year(d), year(d2), month(d)` with a prefix it need not pay.**
  Task 32 step B2's `groupOutputs` is greedy in output order, so the month is offered to the
  ...
  (`PLAN_TASK_32.md` 10.2, 7.6). **Adopted as task 72 (section 2.36)**, whose admission check
  is whether any consumer of a group depends on contiguous output indices.
* **Two calendar outputs over one shift share nothing in production (task 58).** **CLOSED by
  task 32 step B2**: the pair decomposes the same shifted day, so clause 2 of
  `groupOutputs` puts them in one loop method, and the parity row moved from
  813.8 to 1401.8 M rows/s at AVX-512. As written: The
...
* **The parity harness's `dayofweek, chunk 64/63` rows are bimodal on branches after task
  ...
  **Measured, 10 September 2026 (task 77).** Over ten runs per width of an unchanged file the
  `dayofweek, chunk 63` and `chunk 64` rows sit in the band's noisy tier, spreading 27.0% and
  16.6% at AVX-512 with nothing changed at all, so the 23-38% those branch runs showed is not
  evidence of anything a branch did. They are classified as such by
  `dev/varka_bench_diff.py --band`, and a reader is no longer asked to explain them. Why they
  are noisy is milestone 5's task 90; that they cannot be read from a diff is now recorded
