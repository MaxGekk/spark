...
| 76 | The validity helper choice, keyed on the loop body (section 2.38). **Planned** (`PLAN_TASK_76.md`), and its section 2 changes two things the row rested on. The specialised arm is the row named `words per group (task 70 A/B)`, not the task 45 one - the plan's first draft paired it wrongly and briefly concluded the premise had reversed, which it has not. And against task 77's band the single-field loss that motivates the whole task, 8.8% at AVX-512, is *inside* what one of its two rows moves on an unchanged file; at 128-bit the same effect is 7.7% against spreads under 1.5%. So the narrow rows carry the claim and the wide ones can only corroborate | The sweep first, across the number of validity writes a masked loop body makes, at both widths and on the per-group reference arm where the writer is reached at all; then a rule in `planSlots` keyed on that count rather than a second global default, with the `VarkaEmitOptions` switch kept as the reference variant. The plan adds three checks the row did not have. That the two arms still emit different kernels, asserted rather than assumed - this measurement collapsed silently once already when task 70's pass removed the per-group call from both. A ladder in the write count rather than a threshold fitted to the two shapes that raised the question, since those shapes differ in method size and op count as well as in writes. And an explicit decision about the validity *reads*, which are emitted per referenced input rather than per output, so a rule counting writes would otherwise half-apply | The rule keyed on a quantity a ladder chose, not on the two points that raised the question; the crossover read at 128-bit where the band resolves it and reported as `not distinguished` at AVX-512 where it does not; shapes the rule does not reach asserted byte-identical with it on and off; and a decline recorded per width if the effect proves to sit inside the band, which section 2.38 already allows and section 2.3 now makes a width-by-width verdict rather than a single one |
