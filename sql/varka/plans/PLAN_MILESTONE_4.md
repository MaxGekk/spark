...
| 47 | One validity write per word. **Planned** (`PLAN_TASK_47.md`; its section 2 is an admission check that can legitimately close the task, since the destination bitmap is materialised at exactly `(length + 7) / 8` bytes and a 64-bit store on the last word runs off it - the same hazard `validityBitsAt`'s javadoc records for the read side), and **ungated on task 46 as of 10 September 2026**: task 76 was that gate and closed with a decline, so what remains is task 44's non-aligned lengths. Task 76 also handed this row its strongest argument. The per-group read-modify-write is measurably a *4-lane* problem: at 4 lanes a validity group is half a byte, so two consecutive groups rewrite the same byte and serialise on it, and that regime is where the helper choice inverts - width-named helpers win by 8 to 36% at 8 and 16 lanes and lose by 4 to 7% at 4. One write per 64-bit word turns sixteen partial stores over eight bytes into one store and removes the regime rather than tuning inside it | Bits accumulated across lane groups and stored once per 64 rows, with the epilogue flushing a partial accumulator | The masked path's committed cases, the 4095/63 non-aligned lengths task 44 adds, and the dense/masked agreement; gated on what 45 and 46 leave |
