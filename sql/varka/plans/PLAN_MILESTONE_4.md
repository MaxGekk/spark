...
* **The parity harness carries a depressed-row cluster per regeneration.** Every
  ...
  has had to make. Closing it means either finding the cause (the shape of it is a C2
  compilation the JVM finds or does not, the task 50 watch's territory: a per-row register
  spill or a lost vectorisation in one method of a long-running JVM) or making the harness
  run every case in a fresh JVM, which the `dev/varka_bench_regen.sh` provenance would then
  record as the methodology change.
