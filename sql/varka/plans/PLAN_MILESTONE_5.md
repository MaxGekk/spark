...
| 124 | The no-fallback proof: `PrintIntrinsics` in CI (section 2.59). **Planned, instrument re-opened** (`PLAN_TASK_124.md` 2.5, 18 September 2026): measured over real emitted kernels rather than hand-written probes, a healthy Varka kernel emits `** operation not supported` for `VECTOR_OP_MUL` and `VECTOR_OP_AND` on int lanes - operations `SKILLS.md`'s disassembly shows the shipped loop does lower - because the flag logs every compilation *attempt* and the question is about the *final* one. Deterministic, so a per-shape baseline works, but it would pin the status quo rather than prove no fallback; the asm mnemonic scan `dev/varka_emit.sh --asm` already has may be the better instrument. **Planned** (18 September 2026), which corrected 2.59's "How" from measurement: the message to grep for is `** not supported` alone (0 lines clean, 22 refused), `** Rejected` does not exist, `** missing constant` is background, the line carries no method so attribution must be process-level, and a blanket rule would fail the AVX2 arm on `MagicProbe`. The positive control 2.59 asks for already exists as a committed probe. **Scoped** (16 September 2026) | A test that runs the emitter and kernel suites in a forked JVM under `-XX:+UnlockDiagnosticVMOptions -XX:+PrintIntrinsics` and fails on any `** not supported` or `** Rejected` line whose method is a Varka class; a line in `dev/varka_datapath.sh` asserting `EnableVectorSupport=true` from `-Xlog:compilation` | The Varka engine CI job carries the test; a kernel deliberately given an operation the match rules refuse fails it |
