...
* **Task 45, the null-free fast path. DONE** (`PLAN_TASK_45.md` 8), and it was
  ...
  `orValidityBitsAt` is not emitted. Measured as an A/B in one run: the shared
  four-field kernel goes **812.7 to 1477.4 M rows/s at AVX-512 (+82%)** and
  **274.7 to 626.4 at 128-bit (+128%)**, `year` +26% and +41%, `dayofweek` +8%
  and +48%. The four-field shape now beats the hand-written ceiling it was
  chasing by 2.1x. The forecast below that 45 "does nothing for the masked one"
...
rather than "nearly free": the four-field shape's remaining 1477.4 M rows/s
...
| 45 | The null-free validity fast path. **DONE** (`PLAN_TASK_45.md` 8) - the driver sets a dense batch's value outputs valid once and the loop's per-lane-group `orValidityBitsAt` is not emitted; the shared four-field kernel gains 82% at AVX-512 and 128% at 128-bit, `year` 26% and 41%, `dayofweek` 8% and 48%, with both mixed-null controls inside 2% at both widths. Prediction 3 - that the narrow width gains more, the same argument 2.17 makes for task 46 - is the cleanest hit; prediction 4 held at AVX-512 and missed at 128-bit | The bound first: a validity-free variant of `ChronoVectorOps` sizing the prize (2.17), then the dense driver filling the output validity once per batch instead of the dense loop ORing it per lane group | The whole Varka suite at both widths with the dense/masked pair still agreeing bit for bit; the committed parity cases regenerated in one run, with the null-free and mixed-null rows of each moving in opposite directions or not at all |
