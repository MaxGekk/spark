...
| 71 | `GROUP_BUDGET` retuned on the emitter as it is (section 2.35). **Planned** (`PLAN_TASK_71.md`), and its section 2 finds the row's question narrower than the situation. The reversal is confirmed rather than assumed: over task 77's ten runs per width of an unchanged file the merged arm leads in ten of ten at both widths, ratio 1.29-1.33, on rows in the band's quiet tiers - so the shipped default loses about 31% on the one shape it was chosen from, and it is not noise. But `groupOutputs` has two bounds, and a group sharing a calendar prefix already reaches 400 ops in one method while a group sharing an arithmetic chain is held to 16. Task 17's pair is the second kind, so the variable may be what clause 2's `saved` counts rather than the number | A budget ladder (16 to 64) over the shapes a wider method changes, at both widths, with `-XX:+PrintCompilation` beside the throughput; C1's ~1900-byte refusal priced as a startup cost; whether B2's clause 2 widens from prefix reuse to "the marginal cost fits" decided on the same ladder. The plan adds three things the row did not have. A mechanism probe first, because the javadoc's own reading - that task 17's loss was a refused `orValidityBitsAt` call the reordering later inlined, not register pressure - is a hypothesis with no direct evidence, and picking a number without settling it is picking a number. Clause 2's reach as a measured arm beside the budget, since a rule that makes the number irrelevant on these shapes is a better answer than a bigger number. And the budget-identity guard section 2.35 believes already exists: B2's byte-identity test compares `shareChronoPrefix` off against on at one budget, so nothing today asserts that a budget change moves only the shapes it names | The ladder read only where the band permits it - of the shapes 2.35 names, only task 17's pair is quiet enough to settle a small effect and the DAG-CSE row is unreadable at AVX-512, so a rung whose effect lands inside a row's tier is reported as not distinguished rather than ranked; compile time measured beside throughput, since C1 refuses a loop method past about 1900 bytes and it then runs interpreted until C2 lands, which a throughput ladder cannot see; and the cost of a default change enumerated before it is chosen, because roughly two dozen pinned oracles address `loopDense0`/`loopMasked0` by name and `VarkaAssemblySuite` pins those names inside expected C2 frames |
