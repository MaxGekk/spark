...
first batch, usually within a second or two, and the batches that took Spark's
row path meanwhile are counted in `numWarmupBatches`
(`spark.sql.codegen.varka.warmup.enabled`). Anything else fell back to stock
