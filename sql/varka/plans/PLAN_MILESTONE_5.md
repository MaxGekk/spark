...
operations, and in a dense body it costs what five operations cost: 2.8% on
`i + 1` at AVX-512 and 25.7% at 128-bit (`VarkaArithmeticBenchmark`). In a
masked body the same node costs 66.0% at 128-bit - 6459.9 M rows/s against
18986.9 with the check off - on arithmetic that did not change. The
...
by narrowing the word rather than accumulating it, is slower again - 4098.7
against the wrapping add's 19224.5 at 128-bit.
