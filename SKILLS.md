...
The contents below are generated by `dev/varka_toc.py`, which the pre-commit hook runs
whenever this file changes. Add a lesson as a new `##` section and regenerate.

<!-- BEGIN generated contents -->
* [Classpath Shadowing (the stub trap)](#classpath-shadowing-the-stub-trap)
* [Buffer-Reuse Aliasing (UnsafeProjection)](#buffer-reuse-aliasing-unsafeprojection)
* [Alias Unwrap Is Needed at Two Layers](#alias-unwrap-is-needed-at-two-layers)
* [Masked Bugs](#masked-bugs)
* [Debugging Method: Progressive Isolation](#debugging-method-progressive-isolation)
* [Environment Facts (verified in this repo)](#environment-facts-verified-in-this-repo)
* [Columnar Transition Wiring (plan level)](#columnar-transition-wiring-plan-level)
* [Metrics as the "did it really run" proof](#metrics-as-the-did-it-really-run-proof)
* [Extra Sessions on the Shared Context](#extra-sessions-on-the-shared-context)
* [Build Gotchas](#build-gotchas)
* [Build Performance (measured, Aug 2026)](#build-performance-measured-aug-2026)
* [C2 Compile Latency Is the Wide-Vector-Loop Cliff (root cause, proven)](#c2-compile-latency-is-the-wide-vector-loop-cliff-root-cause-proven)
* [A hand-written comparison kernel needs every fast path the real one has](#a-hand-written-comparison-kernel-needs-every-fast-path-the-real-one-has)
* [Calling into an uncompilable method costs something even when it does nothing](#calling-into-an-uncompilable-method-costs-something-even-when-it-does-nothing)
* [A benchmark that reuses `repeats` for wall-clock scaling must also scale its declared row count](#a-benchmark-that-reuses-repeats-for-wall-clock-scaling-must-also-scale-its-declared-row-count)
* [Sharing below the node level in an emitter](#sharing-below-the-node-level-in-an-emitter)
* [Vector API on HotSpot, Measured (JDK 25, x86-64)](#vector-api-on-hotspot-measured-jdk-25-x86-64)
* [Generated Code Can Carry Its Own Debug Info (Class-File API)](#generated-code-can-carry-its-own-debug-info-class-file-api)
* [The Class-File API's Stack-Map Generator Is a Free Verifier](#the-class-file-apis-stack-map-generator-is-a-free-verifier)
* [Independent Reference Evaluators as Test Oracles](#independent-reference-evaluators-as-test-oracles)
* [A fixture that fills undefined memory decides what its whole matrix can catch](#a-fixture-that-fills-undefined-memory-decides-what-its-whole-matrix-can-catch)
* [A count of calls on an owner is not a count of the work you mean](#a-count-of-calls-on-an-owner-is-not-a-count-of-the-work-you-mean)
* [An inventory made by reading is not an inventory made by counting](#an-inventory-made-by-reading-is-not-an-inventory-made-by-counting)
* [What "the masked method is the dense method's bytes" actually took](#what-the-masked-method-is-the-dense-methods-bytes-actually-took)
* [A default decided from a regeneration costs a second regeneration](#a-default-decided-from-a-regeneration-costs-a-second-regeneration)
* [Testing Under AQE](#testing-under-aqe)
* [The benchmark controls are necessary and not sufficient](#the-benchmark-controls-are-necessary-and-not-sufficient)
* [A `--rounds` probe with no nulls compiles only half the kernel](#a---rounds-probe-with-no-nulls-compiles-only-half-the-kernel)
* [Flipping a default silently retires every A/B built on `DEFAULTS`](#flipping-a-default-silently-retires-every-ab-built-on-defaults)
* [Write the Prediction Down, Then Measure](#write-the-prediction-down-then-measure)
* [Reading a paper into the repo](#reading-a-paper-into-the-repo)
* [A reciprocal's top bits are a remainder, and other things a neighbouring codebase had](#a-reciprocals-top-bits-are-a-remainder-and-other-things-a-neighbouring-codebase-had)
* [Validate a fixed-format string with a saturating subtraction](#validate-a-fixed-format-string-with-a-saturating-subtraction)
* [Velox is a semantics reference for the calendar family, not a performance one](#velox-is-a-semantics-reference-for-the-calendar-family-not-a-performance-one)
* [The Julian map: one division stage fewer in civil-from-days](#the-julian-map-one-division-stage-fewer-in-civil-from-days)
* [A GPU port of Spark's date code is scalar code run per thread](#a-gpu-port-of-sparks-date-code-is-scalar-code-run-per-thread)
* [Every operator the plans rely on is one instruction; two species in one JVM is a box per iteration](#every-operator-the-plans-rely-on-is-one-instruction-two-species-in-one-jvm-is-a-box-per-iteration)
* [A range guard belongs where the value is made, not where it is read](#a-range-guard-belongs-where-the-value-is-made-not-where-it-is-read)
* [A derived input must never raise, because the row engine's null check comes first](#a-derived-input-must-never-raise-because-the-row-engines-null-check-comes-first)
* [A closed `ArrowBuf` still answers `capacity()` and `memoryAddress()`](#a-closed-arrowbuf-still-answers-capacity-and-memoryaddress)
* [A recipe for a cheap agent ages at the rate of the emitter, not of the arithmetic](#a-recipe-for-a-cheap-agent-ages-at-the-rate-of-the-emitter-not-of-the-arithmetic)
* [A number superseded inside its own PR loses its provenance when the PR squash-merges](#a-number-superseded-inside-its-own-pr-loses-its-provenance-when-the-pr-squash-merges)
* [Repo Workflow (vecbricks/varka)](#repo-workflow-vecbricksvarka)
* [A store the loop repeats per group with a constant operand is a fill the driver should do once](#a-store-the-loop-repeats-per-group-with-a-constant-operand-is-a-fill-the-driver-should-do-once)
* [A refused call is refused by the caller's budget, and the caller's budget is spent in program order](#a-refused-call-is-refused-by-the-callers-budget-and-the-callers-budget-is-spent-in-program-order)
* [Read two fields out of one product, and put the axis where the formula wants it](#read-two-fields-out-of-one-product-and-put-the-axis-where-the-formula-wants-it)
* [A bimodal kernel is usually the register allocator, and here is how to prove it](#a-bimodal-kernel-is-usually-the-register-allocator-and-here-is-how-to-prove-it)
* [Watching what C2 compiled, at runtime, with no flags](#watching-what-c2-compiled-at-runtime-with-no-flags)
* [This machine's AVX-512 is 256 bits wide, and every "512-bit" number in this repo is really a 256-bit one](#this-machines-avx-512-is-256-bits-wide-and-every-512-bit-number-in-this-repo-is-really-a-256-bit-one)
* [A benchmark guard that fails in one direction gets satisfied by the failure in the other](#a-benchmark-guard-that-fails-in-one-direction-gets-satisfied-by-the-failure-in-the-other)
* [Building a fastdebug JDK for HotSpot diagnostics](#building-a-fastdebug-jdk-for-hotspot-diagnostics)
* [The shape a test picked because nothing lowered it](#the-shape-a-test-picked-because-nothing-lowered-it)
* [A benchmark number is reproducible within a run and not between runs](#a-benchmark-number-is-reproducible-within-a-run-and-not-between-runs)
* [A value that depends on repo state is queried, not chosen](#a-value-that-depends-on-repo-state-is-queried-not-chosen)
* [An overflow check is nearly free in wide lanes and expensive in narrow ones](#an-overflow-check-is-nearly-free-in-wide-lanes-and-expensive-in-narrow-ones)
* [A local nothing reads is not free, and its cost is width-dependent](#a-local-nothing-reads-is-not-free-and-its-cost-is-width-dependent)
* [A checklist for the next node type or mode, from what three reviews found in this one](#a-checklist-for-the-next-node-type-or-mode-from-what-three-reviews-found-in-this-one)
* [A refusal shared by two positions carries one reason, and it can be true of only one](#a-refusal-shared-by-two-positions-carries-one-reason-and-it-can-be-true-of-only-one)
* [Check that the place a prediction blames actually exists](#check-that-the-place-a-prediction-blames-actually-exists)
* [Two things share the name `uncommon_trap`, and only one of them happened](#two-things-share-the-name-uncommon_trap-and-only-one-of-them-happened)
* [A band says which moves to read; an invariant says which to stop the line for](#a-band-says-which-moves-to-read-an-invariant-says-which-to-stop-the-line-for)
* [A budget that bounds the method is not a budget that bounds the work](#a-budget-that-bounds-the-method-is-not-a-budget-that-bounds-the-work)
* [A default that is wrong at one width may be wrong for a reason the width creates](#a-default-that-is-wrong-at-one-width-may-be-wrong-for-a-reason-the-width-creates)
