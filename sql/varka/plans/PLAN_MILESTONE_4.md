...
| 77 | The 128-bit compile cliff behind the per-group validity OR (section 2.39). **Planned** (`PLAN_TASK_77.md`), and re-scoped: 2.39's correction of 10 September 2026 retires both the cause the row was written on and the lowering it is named after. A controlled pair inside one run - benches 930 and 945, one expression with the bitmap pass and with `perGroupWrite` forcing the OR - storms identically, so the OR is not what makes a kernel recompile; and the row this section is about has recovered, while a storm of unknown cause continues elsewhere, worst in a *hand-written* kernel at 100 deoptimisations across 194 compiles. The task therefore takes the operational half and leaves the cause to its own row | Make a regeneration readable without running it twice, which is the tax the debt register has recorded since task 52. Measure what an unchanged file does, per case and per width, and commit it (`dev/varka_bench_band.py`, ten runs per width, split-half to decide whether a per-case band reproduces at all or only a file-level threshold is defensible); have `dev/varka_bench_diff.py` classify each move against it; and gate the collapses that are not noise with an assertion rather than a number - the `Relative` column already says when a reference arm beats the arm it exists to lose to, so the check reads the committed file and touches neither the emitter nor Spark's benchmark harness. Plus the keying fix both need: `(table, case)` is not unique in this file and a collided key is dropped from `--requote` | The split-half answer recorded either way, and the artefact matching it rather than inventing per-case precision; the gate's reference list enumerated from the file with its completeness asserted; and the gate walked back through every committed parity file in git history, flagging the `aef0b82260e`-to-task-70 range and no other revision, which is what turns 'a committed row that would have caught the collapse the day it landed' into a checked claim |
