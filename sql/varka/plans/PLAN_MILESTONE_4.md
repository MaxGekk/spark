...
| 68 | Year-month interval algebra (section 2.33). **Planned** (`PLAN_TASK_68.md`); its section 2 splits this row's list in three - the algebra that rides task 63's nodes, the two divisions that do not, and the two shapes task 67 left, which are blocked on an emitter check rather than on arithmetic. The task takes the first and third; the divisions become their own row, since an int-lane magic for `/12` is exact only over 0..49,151 against the type's full int32 range, and both milestone 5's task 65 and its new task 88 would remove that bound outright - they are milestone 5's task 89, sequenced after whichever route the A/B chooses, with `extract(MONTH)`'s `ByteType` output as a blocker neither route removes | `make_ym_interval`, `extract(YEAR | MONTH FROM ym)` by literal-divisor magic, `ym * k` and `ym / k` with a literal, `ym +- ym`, `-ym`, `abs(ym)` on task 63's nodes with an interval output, the `YEAR`-unit casts; the literal-divisor node that section 2.30's note and scope item 11 both want | The admission check on the rounding of the literal division and the exactness range of the magic divisions over the whole int32 month range; the differential in both ANSI modes with the overflow rows raising the row engine's own error; a parity row per new node beside task 63's; both pinned fixtures re-pinned once |
