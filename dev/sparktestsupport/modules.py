...
varka_bench = Module(
    ...
    source_file_regexes=[
        "sql/varka/bench/",
        # The shell drivers that submit the bench module to a distribution and check the
        # machine it runs on. They are the bench job's own tooling and nothing else's, and
        # like the files above they matched no module until now, so editing one ran the whole
        # matrix. The developer-only scripts - the gate, the pre-commit hook, the worktree
        # helper - are deliberately not here: no CI job runs them, so the honest answer for
        # those is still `root` until someone shows what they should select instead.
        r"dev/varka_bench_[a-z_]+\.sh$",
        r"dev/varka_datapath\.sh$",
    ],
    sbt_test_goals=[],
)
...
catalyst = Module(
    ...
    source_file_regexes=[
        "sql/catalyst/",
        # Two committed files that live outside a module's tree and belong to catalyst suites:
        # `coverage.json` is generated by `VarkaCoverageSuite` and byte-compared by it, and
        # `emitted_bytes.json` is the emitted-bytes oracle's committed digests. Both are under
        # `sql/varka/`, which no module claimed, so a change to either matched nothing, fell
        ...
        r"sql/varka/coverage\.json$",
        r"sql/varka/emitted_bytes\.json$",
    ],
    sbt_test_goals=[
        "catalyst/test",
    ],
