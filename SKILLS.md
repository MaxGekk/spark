...
exactly that, every early sample suspect and the tail clean at a constant 520 bytes per call, short
of a million rows). Batch 512 first, then powers of two and every 4096th - two million rows at the
default batch size; suspect above 4 KB plus one byte per row; one warning per task on two consecutive
suspect samples, a `numSuspectAllocationSamples` metric, and a `KernelAllocation` JFR event on every
sample. The decisions live in `VarkaAllocationSampler` and are unit-tested against a loop that
